...
class Generator {
	...
	public:
		...
		const size_t size;
		Generator(const std::string & p_name, const size_t & p_size, const uint16_t & p_percent_read, Events & p_events, Server * const p_server);
...
class RateGenerator : public Generator {
	...
	public:
		RateGenerator(const std::string & name, const size_t & p_size, const uint16_t & percent_read, Events & events, const unsigned int & ia_time, Server * const server);
...
class QueueGenerator : public Generator {
	public:
		...
		QueueGenerator(const std::string & name, const size_t & p_size, const uint16_t & percent_read, Events & events, const unsigned int & p_qdepth, Server * const server);
	       	void StartTask(const uint64_t & t);
	       	void EndTask(const uint64_t & t);
};
